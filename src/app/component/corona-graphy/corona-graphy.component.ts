...
export class CoronaGraphyComponent implements OnInit {
  ...
  coronaContry: any;
  coronaStatistic: any;
  listCountryCorona: any;
  constructor(private coronaService: CoronaService) {}

  ngOnInit() {
    // list info death, confirm, recovered of corona
    this.coronaService.getInfoCorona(0, 200).subscribe(
      (data) => {
    ...
    this.coronaService.getTotal().subscribe(
      (data) => (this.coronaTotal = data),
      (error) => (this.errorMessage = error)
    );

    // get corona by country
    this.coronaService.getCoronaByCountry("Cyprus").subscribe(
      (data) => (this.coronaContry = data),
      (error) => (this.errorMessage = error)
    );

    // get statistic corona
    this.coronaService.getStatisticCorona().subscribe(
      (data) => (this.coronaStatistic = data),
      (error) => (this.errorMessage = error)
    );

    // get all country corona
    this.coronaService.getAllCountryCorona().subscribe(
      (data) => (this.listCountryCorona = data),
      (error) => (this.errorMessage = error)
    );
  }
}
