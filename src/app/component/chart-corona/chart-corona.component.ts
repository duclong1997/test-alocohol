...
export class ChartCoronaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.coronaService.getStatisticCorona().subscribe(data => 
      {this.liststatisticCoro = data;
        ...
        for(let i= 0; this.listCoronaFeatures.length;i++){
          this.lineChartLabels[i] = this.listCoronaFeatures[i].attributes.Report_Date_String;
          this.listMainChina[i] = this.listCoronaFeatures[i].attributes.Mainland_China;
          this.listOtherLocation[i] = this.listCoronaFeatures[i].attributes.Other_Locations;
        }
      },
