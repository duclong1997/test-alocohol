...
export class CoronaCountryComponent implements OnInit {
  ...
  private coronaCountry : any;
  private errorMess : any;
  private arrayCoronaCountry = [];
  private coronaInfor :any;
  constructor(private coronaService :CoronaService) { }

  ngOnInit() {
    this.coronaService.getAllCountryCorona().subscribe(data => 
      {
        ...
        // compare giữa 2 date
        this.arrayCoronaCountry.sort((left, right)=> new Date(right.attributes.Last_Update).getTime() - new Date(left.attributes.Last_Update).getTime() );
